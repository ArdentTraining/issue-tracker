...
function unroutedDigest() {
  ...
  if (!recent.length) {
    ...
    recent.slice(0, 12).forEach(function (i) {
      var days = Math.floor((now - new Date(i.submitted_at)) / day);
      var who = String(i.category || '').toLowerCase() === 'course_error' ? 'course' : (String(i.category || '').toLowerCase() === 'shipping' ? 'shipping' : 'tech');
  ...
  if (older) {
    lines.push('');
    lines.push('_Also ' + older + ' older open issues nobody has picked up, most of them course fixes carried over from the old log._');
  }
...
function chatwootEmailFor_(rec) {
  ...
  if (!found) {
    ...
    var search = function (q, byPhone) {
      try {
        // Chatwoot's contact search by name sometimes answers 422 "took too
        // long"; a second go a moment later nearly always works.
        var res;
        try { res = chatwootCall_('/contacts/search?q=' + encodeURIComponent(q)); }
        catch (e1) { Utilities.sleep(1500); res = chatwootCall_('/contacts/search?q=' + encodeURIComponent(q)); }
...
var CODE_STAMP = 'r162 · 2026-09-12';
